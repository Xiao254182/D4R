...
echo "export PATH=$PATH:/usr/local/go/bin" >> /etc/profile && echo "export GOPROXY=https://goproxy.io,direct" >> /etc/profile && source /etc/profile
...
chmod +x /opt/d4r/d4r && cp /opt/d4r/d4r /usr/local/bin/
echo "使用d4r进入系统"
